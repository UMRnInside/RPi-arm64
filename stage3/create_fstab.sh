...
case $FSTYPE in
    ext4|f2fs)
        echo "$ROOT_PART /   $FSTYPE    discard,noatime  0   1" >> $ROOT_PATH/etc/fstab
        ;;
    ext3|ext2)
        ...
        ;;
    btrfs) 
        echo "$ROOT_PART /   $FSTYPE    ssd,discard,noatime  0   1" >> $ROOT_PATH/etc/fstab
        ;;
    *)
        echo "Rootfs using $FSTYPE, make sure it is a valid value."
        echo "$ROOT_PART /   $FSTYPE    noatime  0   1" >> $ROOT_PATH/etc/fstab
        ;;
esac
